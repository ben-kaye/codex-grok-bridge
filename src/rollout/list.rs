//! Thread listing and discovery, adapted from `codex-core/src/rollout/list.rs`.
//!
//! Stripped: `state_db` (no SQLite), `codex_file_search` (simple fs walk instead).
//! Changed: `pub(crate)` → `pub` for gateway access.
...
pub async fn get_threads(
    ...
    let root = gateway_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.codex-acp-gateway/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
/// Locate a recorded thread rollout file by its UUID string.
///
/// Simplified from Codex's version: no state_db or file_search — just walks
/// the `sessions/` directory tree looking for a filename containing the UUID.
pub async fn find_thread_path_by_id_str(
    ...
    let root = gateway_home.join(SESSIONS_SUBDIR);
    find_rollout_by_uuid_in_dir(&root, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    let root = gateway_home.join(ARCHIVED_SESSIONS_SUBDIR);
    if !root.exists() {
        return Ok(None);
    }
    // Archived dir is flat — just scan files directly.
    let mut dir = tokio::fs::read_dir(&root).await?;
    while let Some(entry) = dir.next_entry().await? {
        if entry
            .file_type()
...
/// Walk sessions/YYYY/MM/DD/ looking for a rollout file whose name contains the UUID.
async fn find_rollout_by_uuid_in_dir(root: &Path, id_str: &str) -> io::Result<Option<PathBuf>> {
    if !root.exists() {
        return Ok(None);
    }
    let year_dirs = collect_dirs_desc(root, |s| s.parse::<u16>().ok()).await?;
